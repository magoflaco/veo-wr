...
function report(pct, label) {
    browser.runtime.sendMessage({ action: 'PROCESSOR_PROGRESS', pct, label }).catch(() => { });
...
async function main() {
    try {
        ...
        const data = await browser.runtime.sendMessage({ action: 'GET_VIDEO_BLOB' });
        if (!data || data.error) {
            browser.runtime.sendMessage({ action: 'PROCESSOR_ERROR', error: data?.error || __t('proc_error_novideo') });
            return;
        }

        const { videoData, settings, filename } = data;
        ...
        browser.runtime.sendMessage({ action: 'PROCESSOR_DONE' });
    } catch (err) {
        ...
        browser.runtime.sendMessage({ action: 'PROCESSOR_ERROR', error: err.stack || err.message || String(err) });
    }
}
